                ...
                <div class="row">
                    <div class="col-sm-8 col-lg-8">
                        <form class="needs-validation" novalidate method="POST"
                            action="{{ route('docente.update.tema', $tema->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="form-group">
                                    <label class="form-label"><span class="text-danger">*</span> Titulo:</label>
                                    <input type="text" name="nombre" placeholder="Ingrese el titulo del tema"
                                        value="{{ $tema->tema }}" class="form-control" required>
                                    @error('nombre')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="descripcion"
                                        id="descripcionId">{!! $tema->descripcion !!}</textarea>
                                    @error('descripcion')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="text-center">
                                <a type="button" class="btn btn-danger"
                                    href="{{ route('cursos.curso', $tema->curso_id) }}">Volver</a>
