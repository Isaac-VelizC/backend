...
<div class="conatiner-fluid content-inner mt-n5 py-0">
    ...
    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-start">
                                ...
                                <a class="badge rounded-pill bg-light text-dark"
                                    href="{{ route('docente.borrar.file', $file->id) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <strong>Receta compartida: </strong>
                            <a href="{{ route('admin.show.receta', [$trabajo->receta->id]) }}">{{$trabajo->receta->titulo}}</a>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <form class="text-black" method="POST" action='{{ $isEditing ? route('docente.update.trabajo', $trabajo->id) : route('guardar.tarea.new') }}' enctype="multipart/form-data">
            ...
            @endif
            <div class="text-center">
                <button type="submit" class="btn btn-secondary btn-sm">{{ $isEditing ? 'Actualizar' :
                    'Guardar'}}</button>
                <a type="button" class="btn btn-outline-danger btn-sm"
                    href="{{ route('cursos.curso', $id)}}">Cancelar</a>
            </div>
            <hr>
            <div class="row">
                <div class="col-xl-12 col-lg-12">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control" name="titulo"
                                        value="{{ $isEditing ? $trabajo->titulo : '' }}" placeholder="Dale un titulo"
                                        required>
                                    @error('titulo') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="criterio">Criterio / Categoría</label>
                                    <select name="criterio" id="criterio" class="form-control" required>
                                        @foreach($criterios as $criterio)
                                            @if($criterio->categorias->isEmpty())
                                                <option value="{{ $criterio->id }}" data-type="{{ $criterio->type }}" @if ($isEditing)
                                                    {{ $trabajo->criterio_id == $criterio->id ? 'selected' : '' }}
                                                @endif>{{ $criterio->nombre }}</option>
                                            @else
                                                <optgroup label="{{ $criterio->nombre }}">
                                                    @foreach($criterio->categorias as $categoria)
                                                        @if($categoria->asistencia == false)
                                                            <option value="{{ $categoria->id }}" data-type="{{ $categoria->type }}" @if ($isEditing)
                                                                {{ $trabajo->catCritTrabajos && $trabajo->catCritTrabajos->cat_id == $categoria->id ? 'selected' : '' }}
                                    ...
                                    <input type="hidden" name="tipo_categoria" id="tipo_categoria" value="{{ $categoria->type }}">
                                </div>
                                <div class="form-group col-md-4">
                                    <label class="form-label">Fecha Limite: *</label>
                                    <input type="datetime-local" class="form-control" name="fin"
                                        value="{{ $isEditing ? $trabajo->fin : '' }}"
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <select class="form-control" name="tema">
                                        ...
                                        <option value="{{$item->id}}" {{ $isEditing && $item->id == $trabajo->tema_id ?
                                            'selected' : '' }}>
                                            {{ $item->tema }}
                                        </option>
                                        @endforeach
                                ...
                                <div class="form-group col-md-8">
                                    <label class="form-label">Opciones del Trabajo:</label>
                                    <div class="row">
                                        <div class="form-group col-md-4">
                                            <input type="radio" name="tipo_trabajo" value="Practico"
                                                class="form-check-input" {{ $isEditing && $trabajo->tipo == 'Practico' ?
                                            'checked' : 'checked' }}>
                                            <label class="form-check-label">Práctico</label>
                                            <input type="radio" name="tipo_trabajo" value="Teorico"
                                                class="form-check-input" {{ $isEditing && $trabajo->tipo == 'Teorico' ?
                                            'checked' : '' }}>
                                        ...
                                        <div class="form-group col-md-4">
                                            ...
                                            <input class="form-check-input" type="checkbox" name='evaluacion' value="1"
                                                {{ $isEditing && $trabajo->evaluacion ? 'checked' : '' }}>
                                        ...
                                        <div class="form-group col-md-4">
                                            ...
                                            <input class="form-check-input" type="checkbox" name='con_nota' value="1" {{
                                                $isEditing && $trabajo->con_nota ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name='descripcion' id="editorTarea"
                                        rows="5">{{ $isEditing ? $trabajo->descripcion : '' }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    var inputHidden = document.getElementById('tipo_categoria');
    // Agregar un evento de cambio al select
    selectElement.addEventListener('change', function() {
        // Obtener el valor del atributo "data-type"
        var selectedType = this.options[this.selectedIndex].getAttribute('data-type');
        
        // Asignar el valor obtenido al input hidden
        inputHidden.value = selectedType;
    });
</script>
